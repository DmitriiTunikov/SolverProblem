...
MainWindow::MainWindow(QWidget *parent) :
    ...
    m_ptrProblemBrocker(NULL), m_ptrSolverBrocker(NULL)
{
    ui->setupUi(this);
...
void MainWindow::onSolve()
{
    ...
    statusBar()->showMessage(QString("Solved."));
...
{
    // Clearing old SolverLibrary
    if (m_ptrSolverBrocker)
    {
        m_ptrSolverBrocker->release();
        m_ptrSolverBrocker = NULL;
        m_ptrSolver = NULL;
    }
    ...
    m_ptrSolverBrocker = reinterpret_cast<IBrocker *>(solverBrockerFunc());
    void * interfaceImpl = NULL;
    if (m_ptrSolverBrocker->canCastTo(IBrocker::SOLVER))
    {
        interfaceImpl = m_ptrSolverBrocker->getInterfaceImpl(IBrocker::SOLVER);
    }
    else
    {
...
{
    // Clearing old ProblemLibrary
    if (m_ptrProblemBrocker)
    {
        m_ptrProblemBrocker->release();
        m_ptrProblemBrocker = NULL;
        m_ptrProblem = NULL;
    }
    ...
    m_ptrProblemBrocker = reinterpret_cast<IBrocker *>(problemBrockerFunc());
    void * interfaceImpl = NULL;
    if (m_ptrProblemBrocker->canCastTo(IBrocker::PROBLEM))
    {
        interfaceImpl = m_ptrProblemBrocker->getInterfaceImpl(IBrocker::PROBLEM);
    }
    else
    {
